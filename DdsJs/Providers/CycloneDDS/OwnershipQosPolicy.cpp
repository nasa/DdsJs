...
{
    OwnershipQosPolicyProxy::KindField::Proxy::CppEntity kind;

    if (!dds_qget_ownership(cppVal, &kind))
    {
        throw Napi::Error::New(env, "Internal error: Could not retrieve Ownership QoS policy information.");
    }
    
    jsValOut.Set(OwnershipQosPolicyProxy::KindField::NAME, OwnershipQosPolicyProxy::KindField::Proxy::NewInstance(env, kind));
}
...
OwnershipQosPolicyProxy::FromJs(Napi::Env env, NapiContainer const& jsVal, CppEntity& cppValOut)
{
    ...
    OwnershipQosPolicyProxy::KindField::Proxy::FromJs(env, jsVal.Get(OwnershipQosPolicyProxy::KindField::NAME).As< OwnershipQosPolicyProxy::KindField::Proxy::NapiContainer >(), kind);

    dds_qset_ownership(cppValOut, kind);
}
