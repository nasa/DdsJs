/**
 * \file CycloneDDS/TimeBasedFilterQosPolicy.cpp
...
{
    TimeBasedFilterQosPolicyProxy::MinimumSeparationField::Proxy::CppEntity minimum_separation;

    if (!dds_qget_time_based_filter(cppVal, &minimum_separation))
    {
        throw Napi::Error::New(env, "Internal error: Could not retrieve Time-Based Filter QoS policy information.");
    }

    jsValOut.Set(TimeBasedFilterQosPolicyProxy::MinimumSeparationField::NAME, TimeBasedFilterQosPolicyProxy::MinimumSeparationField::Proxy::NewInstance(env, minimum_separation));
}
...
TimeBasedFilterQosPolicyProxy::FromJs(Napi::Env env, NapiContainer const& jsVal, CppEntity& cppValOut)
{
    ...
    TimeBasedFilterQosPolicyProxy::MinimumSeparationField::Proxy::FromJs(env, jsVal.Get(TimeBasedFilterQosPolicyProxy::MinimumSeparationField::NAME).As< TimeBasedFilterQosPolicyProxy::MinimumSeparationField::Proxy::NapiContainer >(), minimum_separation);

    dds_qset_time_based_filter(cppValOut, minimum_separation);
}
