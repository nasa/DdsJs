...
{
    typename DurabilityQosPolicyProxy::KindField::Proxy::CppEntity kind;

    if (!dds_qget_durability(cppVal, &kind))
    {
        throw Napi::Error::New(env, "Internal error: Could not extract durability QoS information.");
    }
    jsValOut.Set(DurabilityQosPolicyProxy::KindField::NAME, DurabilityQosPolicyProxy::KindField::Proxy::NewInstance(env, kind));
}
...
DurabilityQosPolicyProxy::FromJs(Napi::Env env, NapiContainer const& jsVal, CppEntity& cppValOut)
{
    ...
    DurabilityQosPolicyProxy::KindField::Proxy::FromJs(env, jsVal.Get(DurabilityQosPolicyProxy::KindField::NAME).As< DurabilityQosPolicyProxy::KindField::Proxy::NapiContainer >(), kind);
    dds_qset_durability(cppValOut, kind);
}
